...
export const SeatButton = memo(function SeatButton({ seat, isMyHold, onSeatClick }: SeatButtonProps) {
    const isOtherHold = seat.state === 'HOLD' && !isMyHold;
    ...
    return (
        <Button
            variant="outline"
            className={cn(
                "h-7 w-7 min-w-0 p-0 text-xs rounded shadow-sm transition-all active:scale-90 touch-manipulation font-bold border-transparent",
                seat.state === 'AVAILABLE' && "bg-white border-slate-200 text-slate-700 hover:border-primary hover:text-primary",
                ...
                seat.state === 'OCCUPIED' && "bg-slate-200 border-slate-200 text-slate-400 cursor-not-allowed opacity-60"
            )}
            onClick={handleClick}
            disabled={seat.state === 'OCCUPIED' || isOtherHold}
        >
            {isMyHold && <Check className="h-3 w-3 stroke-[3]" />}
        </Button>
    );
});
